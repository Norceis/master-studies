...
from operators import productN, zadeh_s_norm, calculate_norms
...
class ANFIS:
    ...
    def __init__(self, inputs, training_data: np.ndarray, expected_labels: np.ndarray, operator_function=productN,
                 second_operator_function=zadeh_s_norm,
        ...
        self.second_operator_function = second_operator_function

        self.possible_operator_combinations = []
        for first_operator in [self.operator_function, self.second_operator_function]:
            for second_operator in [self.operator_function, self.second_operator_function]:
                self.possible_operator_combinations.append([first_operator, second_operator])


        # self.tsk = np.ones((self.nodes_number ,self.input_number+1))
        self.tsk = np.random.random((self.nodes_number * len(self.possible_operator_combinations), self.input_number + 1))

        self.op = [operator_init_value] * self.nodes_number * len(self.possible_operator_combinations)
    ...
    def anfis_estimate_labels(self, fv, op, tsk) -> np.ndarray:
        ...
        R = np.hstack([calculate_norms(arguments, op, norms) for norms in self.possible_operator_combinations])
    ...
    def show_results(self, color=None):
        ...
        plot = ax.scatter(self.training_data[0], self.training_data[1], self.training_data[2], c=result)
